...
export async function GET() {
  ...
  const likedArticles = await prisma.newsArticleLike.findMany({
    where: { userId },
    include: { newsArticle: true },
  });

  const interactedArticles = [...likedArticles.map((l) => l.newsArticle)];

  if (interactedArticles.length === 0) {
    // Fallback: return latest news articles if no history
    const latest = await prisma.newsArticle.findMany({
  ...
  const categoryCounts = interactedArticles.reduce(
